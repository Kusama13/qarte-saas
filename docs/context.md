...
### Landing (`/`)
Hero → FeaturesGridSection (**3 cards "0€/0% chez Qarte"** : SMS confirmation+rappel, 0% commission acomptes, programme fidelite digitale — header "[number] [title]" inline puis desc + competitor full-width — grid `grid-cols-1 md:grid-cols-3` (1 col tablette 640-767px pour eviter ecrasement, 3 cols a partir de md 768px)) → FideliteSection → PageProSection → SocialProofMergedSection (4 temoignages reels Farida T./Yam Nails/Ericka B./Lux Beauty by Laila + case study Nail Salon by Elodie fusionnes — grid `md:grid-cols-2 lg:grid-cols-4`) → PricingTransitionSection (copy muscle BK : titre tape "Planity te facture l'agenda, fidelite, site et SMS separement. Qarte bundle au prix d'un seul module Planity", labels Option 1 "19€/mois" / Option 2 "24€/mois" + badge "Économise ~96€/mois") → PricingSectionCondensed → FAQ (12 questions ordonnees, **q14 + q15 en pole position** : "Combien je peux economiser en switchant ?" + "Et si je perds les clients marketplace ?") → Footer. Toutes les sections landing utilisent des emojis au lieu d'icones Lucide. Typo landing : titres de section en Playfair Display italic sur les mots gradient + surlignage indigo. CTAs primary uniformement `bg-gradient-to-r from-indigo-600 to-violet-600` (Hero, FAQ mid, Pricing Tout-en-un, Footer, MobileSticky, FideliteSection). CTA noir `bg-gray-900` reserve au plan secondaire Pricing Fidelite (hierarchie). Paddings reduits py-14 md:py-20.
...
- Hero responsive : single-col jusqu'a `xl` (1280px+) ou bascule en 2-col `[1fr_1.2fr]` (image col elargie pour contenir mockup 580px sans overlap). iPad Pro 13" portrait (1024×1366) reste single-col, evite chevauchement texte/image. Pill social proof au-dessus H1 affiche logos top 5 marchands DB (fallback `t1-t5.jpg`) via `getTopMerchants()` server-side cache 7j. Sous-CTA = 1 seule ligne sobre "Sans carte bancaire · Tes premieres en 5 minutes" (fini l'empilement de 2 lignes avec doublon "800+ pros").
...
- SocialProofMerged : avatars cards utilisent les vrais logos marchands quand match par nom (yam nails / lux beauty / farida / ericka), fallback sur prochains logos top merchants disponibles, dernier fallback `t1-t4.jpg`. Logique de matching dans `src/app/[locale]/page.tsx`. JSON-LD Reviews dans `src/app/layout.tsx` itere sur les 4 testimonials (anciennement 5).
