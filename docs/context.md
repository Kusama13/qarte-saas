...
| `/api/cron/blog-digest` | 06:30 UTC (8h30 Paris ete / 7h30 hiver) | Envoie 1 article de blog tous les 3+ jours min — **uniquement aux merchants inscrits depuis < 21 jours** (`RECENT_SIGNUP_DAYS = 21`, audience onboarding educatif, evite d'arroser toute la base + protege le quota Resend). Source `src/data/blog-articles.ts`. Idempotence via table `blog_email_dispatches` (mig 125). Template `BlogDigestEmail.tsx`. Respecte `canEmail()` et `email_bounced_at`. |
...
Hero mockup : `HeroPersonMockup` (style Bookin Beautiful) — photo pro beauté générée Imagen 4 (fond supprimé via rembg, `public/images/hero-person-4-crop.png`, z-20 au centre), 3 cartes flottantes : **SMS rappel** (top, z-30 — "SMS de rappel envoyé / Le rdv de Lisa est confirmé"), **Rdv du jour** (gauche, z-10, -rotate-6 — 2 prestations avec barre colorée), **Fidélité** (droite, z-10, +rotate-6 — grille 7/10 cœurs rose + barre de progression). Container 580×620px, scale responsive 0.62→1, `min-w-0` sur colonnes grid pour éviter overflow mobile.
