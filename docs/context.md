...
- Dashboard `/dashboard/planning` : 3 onglets (Agenda, Reservations, Parametres) — l'activation resa en ligne a ete fusionnee dans Parametres
  - **Agenda** : vue jour unique (timeline 8h-21h, graduations 15/30/45), pills 7 jours + nav semaine + date picker dropdown, menu kebab pour actions semaine (story Instagram, copie semaine, vider jour/semaine). Mode libre : overlays hachures FERME/PAUSE depuis `merchant.opening_hours`
...
- **Noms de prestations** : affiches en entier (pas de truncate) dans la selection de services, les listes de reservations, les prochains RDV et l'historique client. Truncate conserve uniquement dans la DayView timeline ou l'espace est physiquement limite
