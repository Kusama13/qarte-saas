...
- Helpers partages : `_photo-helpers.ts`, `computeDepositAmount()`, `endTimeFromStart(start, duration)`, `formatDateLong(d, locale)`, `computeDayRevenue(slots, serviceMap)`, `getISOWeekNumber(d)` dans `planning/utils.ts`
