...
### Fidélité sur les fiches de réservation (aucune migration)
- **Objectif** : relier fidélité ↔ résa. Sur la fiche d'un RDV (`BookingDetailsModal`), bande **« Fidélité »** (`LoyaltyBand.tsx`, accent rose/emerald = couleur fidélité du design system) sous les réseaux sociaux : carte à tampons visuelle (pastilles), **récompense prête** mise en avant (encadré + libellé du cadeau), infos rapides (visites, dernière visite, **dépensé**, membre, bons dispo — un item n'apparaît que s'il porte une valeur), hint ludique (« ce RDV complétera sa carte », « +1 à la venue »). Gérée pour `customer_id` null (walk-in → rien).
- **Bouton « ? »** dans l'en-tête de la bande → `Modal` explicatif « Comment les points s'ajoutent » : **Automatique** (scan client + réservation honorée) vs **Manuel** (bouton +/− sur la fiche). Rendu en `fixed` descendant du portal → passe au-dessus de la fiche résa.
- **Montant dépensé** : mode cagnotte → cumul réel `SUM(visits.amount_spent)` (inclut les scans salon) ; mode passage → **dépensé en réservations** = revenu des RDV honorés, **même calcul que l'analytics** (`total_price` snapshot sinon somme des prix prestations + presta perso ; exclut no-show/cancelled en JS pour ne pas écarter les NULL). Masqué si 0.
- **Endpoint** `GET /api/customers/loyalty-snapshot?customerId=&merchantId=` (auth merchant, propriété vérifiée) : progression (`getLoyaltyProgress`), nb visites, dernière visite, dépensé, membre, bons, récompenses, + données pour ouvrir la fiche complète.
- **Helper partagé** `src/lib/loyalty-progress.ts` : `getLoyaltyProgress(card, merchant, tier1Redeemed?)` (état courant : tampons X/Y, restant, récompense prête, tier2, cagnotte + cashback) + `computeTier1Redeemed(redemptions)`. Centralise la logique jusque-là dupliquée (dashboard clients, carte cliente). Distinct de `projectBookingLoyalty` (projection +1, vitrine).
- **Badge liste** : dans `ReservationsSection`, pastille émeraude **« Récompense prête »** (Gift) sur les résas dont la cliente a une récompense à réclamer. `/api/planning` GET attache `slot.loyalty` (stamps/amount) **uniquement quand `booked=true`** (la liste des résas ; grilles jour/semaine ne le consomment pas → pas de requête sur ce hot path).
- **« Voir la fiche complète »** ouvre le `CustomerManagementModal` **directement** depuis le planning (porté dans `document.body` via `createPortal` pour passer au-dessus de la fiche résa), sans naviguer ni rechercher. Le snapshot fournit carte + identité.
- **Agenda** (`DayView`/`WeekView`) : le **nom de la cliente est prioritaire** sur un créneau réservé (affiché dès qu'il y a un minimum de hauteur), la prestation passe en dessous. Créneau libre → prestation comme avant.

### Rappel de RDV par email + infos pratiques (mig 183)
- **Besoin** : certains merchants veulent envoyer des détails à la cliente avant le RDV (adresse & accès, parking, « venez les cheveux propres », arriver 5 min avant). Champ **global** `booking_reminder_details` (≤ 600 car., pas par prestation — décision produit).
- **Canal principal — email de rappel la veille** : cron dédié `booking-reminder-email` (`0 7 * * *`, ~9h Paris) envoie un email la **veille au matin** aux créneaux du lendemain (fuseau merchant, non-filler, acompte non en attente) qui ont un email (`customer_email` snapshot sinon `customers.email`). Contenu : récap RDV + adresse salon + encart infos pratiques + lien carte fidélité + politique annuler/déplacer. Gate `booking_reminder_email_enabled` (opt-in). Dédup par créneau via `merchant_planning_slots.reminder_email_sent_at` (les emails transactionnels n'ont pas de table de log type `sms_logs`). Template `src/emails/BookingReminderEmail.tsx`, wrapper `sendBookingReminderEmail`.
- **Sous-option — aussi dans la confirmation** : case à cocher `booking_reminder_in_confirmation` (sous-option du rappel, sauvée false si le rappel est coupé) → `book/route.ts` passe `practicalDetails` à `sendBookingConfirmationEmail` uniquement si cochée. Boost de couverture (100% des clientes avec email voient l'info au moins une fois).
- **UI partagée email** : composant `src/emails/PracticalDetailsBox.tsx` (encart violet « À savoir avant votre venue ») consommé par le rappel ET la confirmation — source unique, garde anti-débordement (`overflow-wrap`) pour les URLs/codes longs collés.
- **Réglage** : Planning > Paramètres > **Notifs** — carte « Rappel par email la veille » (toggle principal `headerRight` + textarea infos + case à cocher confirmation). Le **SMS J-1 existant** (cron `evening`, 19h) reste inchangé et court ; un renvoi croisé pointe vers Notifications > Automatisations. Couverture email partielle assumée (email facultatif au booking).
...
- **Deux providers cohabitent** depuis avril 2026 :
  ...
  - **Incident passerelle SMS Partner (2026-07-22)** : `api.smspartner.fr` s'est dégradé et répondait un accusé générique `{"success":true,"code":200,"message":"Request queued for processing"}` à **toutes** les requêtes — y compris sans clé API et sur un endpoint inexistant (certificat TLS légitime : c'était bien leur serveur). `message_id` avait disparu de `/v1/send` et l'objet `credits` de `/v1/me`. `sendSmsPartner` ne testait que `data.success === true` → il partait en branche succès avec `jobId: String(undefined)`, marquant **`sent`** des SMS jamais livrés, **sans fallback OVH**. Correctif : un `message_id` exploitable est désormais exigé, sinon échec explicite → classe `unknown` → fallback OVH automatique. Effet de bord non corrigé : `getSmsPartnerCredit()` lit `data.credits.creditSms` et renvoie donc `null` tant que leur API ne le renvoie pas — le cron `sms-credits-check` gère le `null` (pas de fausse alerte) mais **le monitoring du crédit est aveugle** ; à revérifier une fois leur API rétablie.
  - **SMS de demande d'avis gelé** (`REVIEW_SMS_ENABLED`, défaut off, 2026-07-22) — voir [`docs/sms-system.md`](./sms-system.md#gel-du-sms-de-demande-davis-2026-07-22)
...
- **Robustesse SMS transactionnels** (mig 162, mai 2026) : refonte `sendBookingSms` avec fallback intelligent ([sms.ts:sendWithIntelligentFallback](../src/lib/sms.ts)). Sur timeout SMS Partner, **pas de fallback OVH immédiat** (cause de doublons en at-least-once retry) → status `pending_verify` + attente du DLR webhook (`/api/sms-partner/dlr` auth secret query, mig 162) pendant 10min. Si DLR `delivered` → marqué `sent`. Si `not delivered` → fallback OVH inline. Si pas de DLR → cron `sms-verify` (toutes 10min) backstop. Cron `sms-batch-audit` à 19h30 UTC vérifie chaque slot demain a un `reminder_j1` valide, re-envoie défensivement les manquants — **garde-fou** `sms_evening_last_run_at` < 6h pour éviter trigger prématuré (incident 2026-05-08 où trigger manuel à 8h UTC a re-envoyé 9 rappels 11h trop tôt). Numéros confirmés invalides 2 fois sur 2 providers ajoutés à `sms_phone_blacklist` (skip à la source, cache LRU 1000 entries). 8 classes d'erreur ([sms-error-classifier.ts](../src/lib/sms-error-classifier.ts)) : invalid_phone/no_credit/rate_limit/timeout/server_error/config_error décident du fallback. **Maj 2026-07-22** : seul `invalid_phone` est désormais non-fallback-able (un numéro invalide le restera chez OVH) — `config_error` **bascule sur OVH** au lieu de tout couper, car une clé API cassée privait les clientes de leurs confirmations alors qu'OVH fonctionnait. L'alerte admin est préservée via `primaryErrorClass`/`primaryProvider`/`primaryErrorMessage` dans `SendOutcome` : sans ça, un fallback réussi met `errorClass` à `success` et masquerait la clé cassée. Fix critique extraction `errors[0].code` per-numéro chez SMS Partner (top-level `code` vague vs per-numéro précis, ex code 9 mobile invalide). Alertes admin email `sales@getqarte.com` via `notifySmsAdmin()` ([sms-admin-alerts.ts](../src/lib/sms-admin-alerts.ts)) sur config_error (30min dedup) / no_credit (6h) / batch_audit_high_miss (24h). Marketing campagnes + trial marketing **inchangés** : OVH only, pas de fallback. Env var nouvelle : `SMS_PARTNER_DLR_SECRET` (généré + ajouté Vercel ; passé à chaque `urlDlr` de POST `/v1/send`).
...
### Pricing dual-devise
- **FR (EUR) Tout-en-un :** `PLAN` (34€/mois) + `PLAN_SEMESTRIAL` (170€/6 mois — interval=month, interval_count=6) + `PLAN_ANNUAL` (240€/an, **legacy**) — `STRIPE_PRICE_ID` + `STRIPE_PRICE_ID_SEMESTRIAL` + `STRIPE_PRICE_ID_ANNUAL`. **Hausse du 2026-07-23 (24€/120€ → 34€/170€) pour les nouveaux uniquement** : les abonnés en place gardent leur price ID Stripe (grandfathered). Les anciens ID 24€/120€ sont ajoutés à `PLAN_LEGACY_PRICE_IDS` (liste plate multi-générations, valeurs séparées par virgule via `parseIds`) — env `STRIPE_PRICE_ID_LEGACY` (mensuel, on **ajoute** l'ancien 24€ sans écraser) + `STRIPE_PRICE_ID_SEMESTRIAL_LEGACY` (**nouvelle env**, ancien 120€) + `STRIPE_PRICE_ID_ANNUAL_LEGACY`. Le webhook (`tierFromPriceId`) mappe tous ces ID → `all_in`
...
- **getMerchantMonthlyPrice** (utils) retourne prix correct par tier + 2 bornes de date : legacy pre-split (<2026-04-05 = 19€), Fidelite=14€, Tout-en-un=24€/120€ avant le 2026-07-23 sinon 34€/170€. Approximation MRR (pas de prix par-merchant stocké — stopgap, cf. commentaire fonction). Utilise par `/admin/analytics` MRR + tier breakdown.
...
**Budget marketing quotidien** (mig 184, 2026-07-22) : garde-fou central dans `sendEmail` ([`src/lib/email.ts`](../src/lib/email.ts)). Chaque wrapper porte une `category` (`'transactional'` par défaut, `'marketing'` sur les 14 wrappers de relance). Au-delà de `EMAIL_MARKETING_DAILY_CAP` envois marketing sur la journée (fuseau Paris), le marketing est **coupé** (`{ success: false, error: 'marketing_daily_cap' }`) ; le **transactionnel n'est jamais compté ni bloqué**. Compteur atomique table `email_daily_counters` + RPC `increment_email_counter`. Protège le quota Resend (plan gratuit = 100/jour) : sans ça, un blast marketing du matin faisait échouer confirmations de résa et rappels RDV toute la journée. Cap invalide (NaN/négatif) → `Infinity` (plafond désactivé, jamais de blocage accidentel). Lecture du compteur **avant** le rendu react-email.

**Dédup emails — fiabilité** (2026-07-22) : `fetchAllTracking` ([`src/lib/cron-helpers.ts`](../src/lib/cron-helpers.ts)) paginait `pending_email_tracking` avec `.range()` **sans `.order()`** et avalait les erreurs (`data` null → `break` silencieux). Sans ordre stable, la pagination pouvait sauter des lignes → `globalTrackingSet` incomplet → **réenvoi à toute la base**. Incident du 22/07 : 100+ emails « lance ton programme » repartis à des merchants déjà tamponnés (0 ligne de tracking neuve). Corrigé par un ordre total `(merchant_id, reminder_day)` + `throw` sur erreur (fail-safe : planter le cron plutôt que spammer). Helpers `wasEmailSent`/`markEmailSent` ([`src/lib/email-tracking-codes.ts`](../src/lib/email-tracking-codes.ts)) unifient le dédup des 3 routes QR/kit (upsert `ignoreDuplicates`, plus d'insert brut en race).
...
| `/api/cron/email-onboarding` | **19:00 UTC** | **Emails setup progressif** : program reminder J+1, social proof J+3, vitrine J+3, planning J+4, QR code, first client script. **Décalé le soir** (2026-07-22) pour rester proche du reset de quota Resend minuit et ne plus affamer le transactionnel de la journée. **Filtre `canReceiveOnboarding`** : uniquement abonnés `active` ou essais **encore en cours** (`trial_ends_at > now`, sans grace) — 471 essais expirés restés en statut `trial` recevaient les relances en boucle (audience 623 → 55) |
| `/api/cron/email-engagement` | **19:30 UTC** | **Emails engagement** : first scan/booking/reward, tier 2 upsell, inactifs J+7/14/30, referral promo J+2, referral reminders J+14/30, pending points |
...
| `/api/cron/weekly-recap` | **19:00 UTC dimanche** | Push recap semaine a venir aux merchants (X RDV, ~Y€ prevus, 7 jours glissants) |
...
### Landing (`/`)
Hero → FeaturesGridSection (**3 cards** "100/0%/0€" : badge chiffre + titre [SMS inclus / Commission acomptes / Programme fidelite digitale] inline, desc full-width — grid `grid-cols-1 md:grid-cols-3`) → **PageProSection** (acquisition : vitrine + planning + acomptes — titre "Tes résas en ligne, **0% de commission**.") → **FideliteSection** (retention : titre "Le programme de fidélité digitale, **inclus dans ton forfait 34€**." — la ligne "tout seul 14€/mois" a été retirée avec le plan Fidélité, juillet 2026) → SocialProofMergedSection (4 temoignages reels Farida T./Yam Nails/Ericka B./L'Beauty by Lindsay + case study Nail Salon by Elodie fusionnes — grid `md:grid-cols-2 lg:grid-cols-4`, fond `bg-white`) → PricingSectionCondensed → FAQ → Footer. **PricingTransitionSection supprimée** (juillet 2026) : elle justifiait le choix entre 2 plans, devenu sans objet avec le retrait Fidélité. **PricingSectionCondensed = carte unique Tout-en-un** (34€). Ordre cycle client : acquisition → retention → preuves → pricing. Toutes les sections landing utilisent des emojis au lieu d'icones Lucide. Typo landing : titres de section en Playfair Display italic sur les mots gradient + surlignage indigo. CTAs primary uniformement `bg-gradient-to-r from-indigo-600 to-violet-600`. Espacement vertical sections uniformise sur `py-16 md:py-20`.
...
- titlePart2 (rose-500) : "Chez Qarte, c'est 34€."
