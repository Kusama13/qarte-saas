...
| `/api/cron/blog-digest` | 06:30 UTC (8h30 Paris ete / 7h30 hiver) | Envoie 1 article de blog tous les 3+ jours min a tous les merchants opt-in (source `src/data/blog-articles.ts`). Idempotence via table `blog_email_dispatches` (mig 125). Template `BlogDigestEmail.tsx`. Respecte `canEmail()` et `email_bounced_at`. |
