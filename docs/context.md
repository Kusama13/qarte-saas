...
- **Annulation/modification par le client** (mig 096) : `allow_customer_cancel` + `allow_customer_reschedule` (2 toggles independants) + `customer_edit_deadline_days` (delai min en jours avant le RDV). API `DELETE/PATCH /api/planning/customer-edit` (auth cookie phone). Boutons dans `UpcomingAppointmentsSection` sur la carte client. Push + email au merchant. Uniquement pour les RDV `booked_online = true`
