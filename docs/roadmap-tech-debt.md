...
| # | Item | Commit | Mig |
|---|---|---|---|
| **Incident test e2e : 8 reminder_j1 envoyes prematurement + correction** | Pendant le test 5 du sprint robustesse SMS (sms-batch-audit declenche manuellement a 8h UTC pour valider le flow), le cron a detecte 9 slots demain sans reminder_j1 logge et a re-envoye **8 vrais SMS clients** + 1 SMS test. Cause root : `sms-batch-audit` est concu pour tourner a 19h30 UTC apres `evening` (17h UTC) ; declenche avant evening, il croit que les rappels n'ont jamais ete envoyes alors qu'ils sont juste pas encore prevus. **Correction immediate** : DELETE des 8 sms_logs rows incrimines (commit `dda` non commit, action manuelle). Le evening cron normal ce soir 17h UTC va re-envoyer les 8 rappels a l'heure prevue, donc les recipients recoivent 2 SMS au total (matin + soir) mais le merchant ne consomme qu'1 SMS dans son quota (au lieu de 2 si on n'avait pas DELETE). Trade-off assume : 8 doublons recipient pour preserver le compteur merchant. **Garde-fou ajoute (commit `10731c2f`)** : evening cron ecrit desormais `app_config.sms_evening_last_run_at` avec `{ran_at, sent, errors}` a la fin de son run. sms-batch-audit verifie au boot que ce flag a ete update dans les 6 dernieres heures, sinon `skipped: 'evening_not_run_recently'` et early return. Plus possible de declencher un re-envoi premature manuellement. Documente aussi : ne jamais trigger sms-batch-audit a la main avant 17h UTC. Moralite : un test e2e sur prod doit valider le code ET l'orchestration temporelle. | `10731c2f` | — |
| **Vouchers welcome+promo : suppression du pattern voucher pour merchants avec resa en ligne** | Refonte de la sémantique vouchers welcome+offer suite a question user "pourquoi on a encore des vouchers welcome/offer dans la carte fidélité ?". Règle nouvelle : `auto_booking_enabled=TRUE` → JAMAIS de voucher (réduction appliquée direct au booking via `welcome_offer_discount_percent` et `applied_offer_*` mig 153) ; `auto_booking_enabled=FALSE` → voucher pattern conservé tel quel (cas C inchangé, le merchant utilise un autre système de résa et le voucher se consomme en boutique). **6 patches** : (1) `/api/planning/book/route.ts:306` — suppression du bloc legacy `if (welcome_enabled && !welcome_offer_discount_percent) → INSERT voucher` (ce code ne tournait que pour merchants vitrine, donc auto_booking=TRUE par definition → ne devait jamais creer de voucher selon la nouvelle regle). (2) `/api/welcome/route.ts` — voucher creation et referral record gates sur `!merchant.auto_booking_enabled`. (3) `/api/merchant-offers/claim/route.ts` — voucher creation gate sur `!merchant.auto_booking_enabled` (rollback decrement si fail). (4) `VoucherRewards.tsx` — filter UI cache les vouchers `source IN ('welcome', 'offer')` si `merchant.auto_booking_enabled=TRUE` (les vouchers existants en DB expirent naturellement 30j). (5) `WelcomeSection.tsx` (dashboard public-page) — wording `welcomeLegacyHelp` durci en warning amber "⚠️ Action requise : sans pourcentage, ton offre n'a aucun effet". Affiché si toggle ON + auto_booking ON + % vide. (6) `PromoSection.tsx` — nouveau warning amber `promoDiscountMissingWarn` affiche si auto_booking ON + % vide ("⚠️ Action requise : sans pourcentage, ton offre n'aura aucun effet sur les résa en ligne"). Migrations data : aucune, vouchers existants en DB laissés expirer naturellement (UI les cache). Effort : 6 fichiers + 4 cles i18n FR+EN. | (à pusher) | — |
| **Auth reset password : 404 + silent fail + session recovery non purgee** | 3 bugs cumulés sur le flow forgot-password / reset-password. (1) `/auth/callback` était routé via `intlMiddleware` qui tentait de servir `[locale]/auth/callback` (n'existe pas, route handler hors segment locale par design Supabase) → **404 systematique** sur le clic du lien email. Fix double : bypass explicite dans middleware.ts + exclusion du matcher (ceinture+bretelles, protege futurs route handlers `/auth/callback/*`). (2) `auth/callback/route.ts` appelait `exchangeCodeForSession` et `verifyOtp` sans check d'erreur → silent fail si code expiré/déjà consommé/PKCE verifier missing → user redirigé vers reset-password sans session → "Lien invalide" ou crash silencieux du `updateUser`. Fix : check explicite + support du type `recovery` (vs cast force `'signup' | 'email'`) + propagation de l'erreur via `?auth_error=...`. (3) Apres `updateUser({ password })` réussi, la session "recovery" restait active → middleware (`isAuthRoute && session → /dashboard`) détournait le user → message succès jamais vu, nouveau password jamais validé par re-connexion. Fix : `await supabase.auth.signOut()` juste après l'update + lecture du flag `auth_error` au mount pour montrer direct l'écran "Lien invalide". | `3abb9485` + `09e560e6` | — |
| **Diag SMS trial marketing "skipped" — pas un bug, jour ferie** | User signale "les SMS essai ne partent pas notamment example_vitrine". Investigation : `sms_logs` (transactionnel) tourne nickel (519 entries 24h, dernier <1h). SEUL le cron `sms-trial-marketing` skip tout. Vercel logs : `trial_sms_skipped` sur 14 merchants avec `reason: "illegal_time"`. Cause root : **8 mai 2026 = Victoire 1945 = jour ferie FR** hardcodé dans `JOURS_FERIES_FR` (`src/lib/sms-compliance.ts:8`). Loi française interdit SMS marketing les jours fériés → `isLegalSendTime` rejette correctement → cron skip 100% conforme. Pas un bug. **Mini-amélioration livrée** : log enrichi avec champ `detail` (`holiday`/`sunday`/`before_legal_hour`/`after_legal_hour`) au lieu du `illegal_time` opaque, evite faux diagnostic à l'avenir. **Side effect non-fixable** : Meyss Beautyy (créé 7 mai 22:38 UTC) rate son example_vitrine définitivement (signup la veille au soir + jour férié + cap 24h après signup → sortira de la fenêtre éligible avant le prochain cron légal le 9 mai 10h Paris). 3 sur 4 autres merchants éligibles seront rattrapés demain matin. Prochains jours fériés à anticiper : 14 mai (Ascension), 25 mai (Pentecôte). | (à pusher) | — |
| **Blog SEO : article "Acompte salon : 100% direct sur ton compte"** | Nouvel article BOFU `/blog/acompte-rdv-salon-sans-commission` (~1400 mots, 6 min). Angle : Qarte = seule plateforme FR/BE/CH (mai 2026) qui laisse coller son lien Revolut/PayPal/SumUp dans la vitrine, **100% de l'acompte tombe direct sur le compte du merchant** (vs Planity 1,80%+0,18€HT, Booksy 2,49%+0,30$, Treatwell 2%+25% sur nouvelles, Fresha 1,29%+0,15€, Kiute Pro 1,80%+0,18€HT). Tableaux comparatifs + projection 3 ans (écart Booksy vs Qarte = 3 132€). **Process** : senior SEO agent (recherche concurrence + volumes + SERP audit + brief) → rédaction ton beauté chaleureux (vocabulaire concret, bannir "transit"/"business model"/"Stripe Connect") → 3 agents audit en parallèle (copywriting + ai-seo extractability + schema-markup completeness) → v2 fusionnée. **Schema markup graph-linked** (premier article du blog avec ce niveau) : 7 nodes interconnectés via `@id` (Organization + SoftwareApplication avec offers 24€/240€/Trial 7j + WebPage + ImageObject + BreadcrumbList + Article avec mentions des 9 entreprises citées + FAQPage). Pattern à propager sur les 6 autres articles existants. Registry mis à jour (`src/data/blog-articles.ts`), cron `blog-digest` enverra automatiquement. | (à pusher) | — |
| **SMS robustesse : fallback intelligent + DLR webhook + audit J-1 + blacklist + alertes admin** | Refonte profonde du systeme SMS transactionnel pour eliminer (a) les doublons SMS Partner+OVH causes par false-negative timeout, (b) les pertes silencieuses quand les 2 providers timeout simultanement, (c) le gaspillage de credit sur numeros invalides recurrents. **6 nouvelles surfaces** : (1) Mig 162 ajoute `error_class`/`provider_msg_id`/`fallback_attempted_at`/`dlr_received_at`/`verify_after` sur sms_logs + status `pending_verify` + table `sms_phone_blacklist`. (2) Helper `sms-error-classifier.ts` mappe codes provider → 8 classes (success/invalid_phone/no_credit/rate_limit/timeout/server_error/config_error/unknown). (3) `sendWithIntelligentFallback` dans sms.ts : sur timeout SMS Partner, marque `pending_verify` + verify_after=NOW+10min (pas de fallback immediat = pas de doublon) ; sur erreurs claires (no_credit, server_error, rate_limit), fallback OVH immediat ; sur invalid_phone/config_error, abandon direct + side effects (blacklist phone ou alerte admin). (4) Endpoint `/api/sms-partner/dlr` (webhook DLR auth via secret query string `SMS_PARTNER_DLR_SECRET`) : sur `delivered`, marque sent ; sur `not delivered`, fallback OVH immediat ; sur `waiting`, attente. (5) Cron `sms-verify` toutes 10min : backstop pour les `pending_verify` sans DLR (jamais arrive ou perdu). (6) Cron `sms-batch-audit` a 19h30 UTC (21h30 Paris, 2h30 apres reminder_j1 cron evening) : verifie chaque slot demain a un sms_logs reminder_j1 valide, re-envoie defensivement les manquants, alerte admin si > 5% manquants. **Helper `sms-blacklist.ts`** : numeros confirmes invalides 2 fois sur 2 providers ajoutes a la blacklist (pre-check sendBookingSms skip a la source). **Helper `sms-admin-alerts.ts`** : email a `sales@getqarte.com` via Resend pour 5 types d'incidents critiques (config_error 30min dedup, no_credit OVH/SMS Partner 6h dedup, taux d'echec eleve 1h dedup, audit J-1 manquants 24h dedup). Pattern aligne sur sendNewMerchantNotification + sendSmsCreditLowEmail. Stockage app_config jsonb `{sent_at: ISO}`. **OVH timeout 5s→8s + 3 retries** (aligne sur SMS Partner). **Marketing campagnes + trial marketing inchanges** (OVH only, pas de fallback). vercel.json +2 crons. Nouvelle env var `SMS_PARTNER_DLR_SECRET` (a generer + ajouter Vercel + configurer cote SMS Partner dashboard comme `urlDlr=${APP_URL}/api/sms-partner/dlr?secret=...`). 11 fichiers + 1 migration. | (à pusher) | **162** ⚠️ à appliquer |
...
| **Hotfix mig 161 : CHECK constraint `merchant_marketing_sms_logs.sms_type` étendue avec `example_vitrine`** | Bug critique mig 158 : la mig avait ajouté le type `example_vitrine` dans le code TS (`sms-trial-marketing.ts`, cron horaire SECTION 0) + flag dedup sur `merchants.example_vitrine_sms_sent_at`, mais oublié d'étendre la `CHECK` constraint sur `merchant_marketing_sms_logs.sms_type`. Conséquence : 2 merchants signalés dans les logs (signups récents post-mig 158), SMS bien envoyés chez OVH (`status:'sent'`, `0.0750€` débité), mais INSERT log row a échoué avec `23514 violates check constraint`. **Pas de risque de re-envoi** : le flag dedup `example_vitrine_sms_sent_at` est set indépendamment du log row ([sms-trial-marketing.ts:135-140](src/lib/sms-trial-marketing.ts)), donc dedup OK. Seul impact : 2 lignes manquantes dans `/admin/sms` + 0,15€ non comptés dans le cost report. Mig 161 : DROP + ADD CONSTRAINT (avec les 6 types : 5 existants + `example_vitrine`). | (à pusher) | **161** ⚠️ à appliquer ASAP |
...
| 161 | `marketing_sms_logs` CHECK étendue (8 types incluant checkin_nudge/combo + example_vitrine) + catch-up consolidé 124+146+147+149 | ⚠️ à appliquer |
| 162 | `sms_robustness` (sms_logs : error_class + provider_msg_id + fallback_attempted_at + dlr_received_at + verify_after + status pending_verify) + table sms_phone_blacklist | ⚠️ à appliquer |
