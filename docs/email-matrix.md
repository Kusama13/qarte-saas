...
**Nouveaux emails** :
- `ActivationStalledEmail` ([src/emails/ActivationStalledEmail.tsx](../src/emails/ActivationStalledEmail.tsx)) — S0 J+1 case studies + path tier-aware (tracking code -320)
...
**Emails redondants NON supprimés** (ProgramReminderDay2/3, SocialProof, VitrineReminder, PlanningReminder, Day5Checkin) — ActivationStalled les remplace fonctionnellement pour S0 J+1, mais les triggers redondants tournent encore. À cleanup en Phase finale v3.
...
|------|-------------|---------|
| `morning` | 08:00 | Billing-critical : trial ending J-1 (daysRemaining=1), trial expired J+1 grace, reactivation, dunning, incomplete signup T+24h |
| `email-onboarding` | 10:00 | Setup progressif (cadence 3j trial) : program reminder + vitrine J+1, social proof + planning J+2, activation stalled J+1, QR code, first client script |
...
## 2. Onboarding actif (cadence 3j trial — 1 email/jour, priorité dedup)

Envoyes via le **cron `email-onboarding`** (10:00 UTC) aux merchants actifs (trial/active).
Chaque jour, 1 email max par merchant : sections priorisées via `emailedAtJ1`/`emailedAtJ2` Sets.

```
J+1   priorité descendante (1 email max/merchant) :

   1. ACTIVATION STALLED (-320)
      Objet: "{shopName}, 3 jours pour transformer..."
      Condition: configuré + score activation = 0 (S0)

   2. VITRINE REMINDER (-304)
      Objet: "{shopName}, tes futures clientes te cherchent sur Google"
      Condition: configuré + PAS de bio ni adresse + skip si stalled

   3. PROGRAM REMINDER (-301)
      Objet: "{shopName}, ta 1ere cliente attend sa carte"
      Condition: PAS configuré (mutuellement exclusif des 2 ci-dessus)

J+2   priorité descendante (1 email max/merchant) :

   1. PLANNING REMINDER (-308)
      Objet: "{shopName}, tes clientes veulent reserver en ligne"
      Condition: configuré + planning PAS activé + PAS hidden

   2. SOCIAL PROOF (-310)
      Objet: "{shopName}, comment Elodie a doublé ses clientes"
      Condition: tous les trial + skip si Planning déjà envoyé
```
...
## 5. Merchants inactifs (programme configure, abonne actif)
...
## 6. Paiement & Dunning (subscription active → past_due)
...
## 7. Subscription lifecycle (Stripe webhooks, immediats)
...
## 8. Reactivation (apres resiliation)
...
## 10. Emails operationnels (declenchement immediat)
