  ...
  return (
    <div className="relative w-full max-w-[340px] mx-auto">
      <div className="flex gap-4">
        <div className="flex-1 bg-white/[0.06] rounded-2xl shadow-lg shadow-black/20 border border-white/10 p-4 flex flex-col items-center">
          <div className="w-16 h-16 bg-white/10 rounded-xl border-2 border-indigo-500/30 p-2 shadow-sm mb-3">
            <svg viewBox="0 0 100 100" className="w-full h-full">
              ...
              <rect x="10" y="10" width="15" height="15" rx="2" fill="#1e1b4b" />
              <rect x="75" y="10" width="15" height="15" rx="2" fill="#1e1b4b" />
              <rect x="10" y="75" width="15" height="15" rx="2" fill="#1e1b4b" />
        ...
        <div className="flex-1 bg-gradient-to-br from-violet-500/10 to-pink-500/10 rounded-2xl shadow-lg shadow-violet-500/10 border border-violet-500/20 p-4 flex flex-col items-center">
          <div className="relative mb-3">
            <Image
              ...
              width={80}
              height={52}
              className="rounded-lg shadow-md shadow-violet-500/30"
            />
          </div>
...
function ProgramBrandVisual({ t }: { t: (key: string) => string }) {
  ...
  return (
    <div className="relative w-full max-w-[340px] mx-auto">
      <div className="absolute inset-0 -m-4 bg-violet-500/10 rounded-3xl blur-[60px] pointer-events-none" />

      <div className="relative bg-white/[0.06] backdrop-blur-sm rounded-3xl shadow-xl shadow-black/20 border border-white/10 p-5">
        <div className="flex items-center justify-center gap-2.5 mb-4">
          {colors.map((color, i) => (
            <motion.div
              ...
              className="relative"
            >
              <div className={`w-8 h-8 md:w-9 md:h-9 rounded-full ${color.bg} ${color.active ? 'ring-2 ring-white/60 ring-offset-2 ring-offset-gray-950' : ''} shadow-sm`} />
              {color.active && (
                <MousePointer2 className="absolute -bottom-1.5 -right-1.5 w-4 h-4 text-white/70 drop-shadow-sm" />
              )}
            </motion.div>
          ))}
        </div>
      ...
      <div className="space-y-3">
        {reminders.map((notif, i) => (
          <motion.div
            ...
            className="flex items-start gap-3 bg-white/[0.06] backdrop-blur-sm rounded-2xl p-4 shadow-lg shadow-black/20 border border-white/10 hover:bg-white/[0.1] hover:-translate-y-0.5 transition-all duration-300"
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="text-[10px] text-white/30 flex-shrink-0">{notif.time}</span>
              </div>
      ...
      <div className="space-y-3">
        {offers.map((offer, i) => (
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.5, delay: 0.15 + i * 0.1, ease: EASE }}
            className="flex items-center gap-3 bg-white/[0.06] backdrop-blur-sm rounded-2xl p-4 shadow-lg shadow-black/20 border border-white/10 hover:bg-white/[0.1] hover:-translate-y-0.5 transition-all duration-300"
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-white/40">{offer.label}</p>
            </div>
            <div className="flex items-center gap-1 bg-emerald-500/20 rounded-full px-2.5 py-1 flex-shrink-0">
              <Check className="w-3 h-3 text-emerald-400" />
              <span className="text-[10px] font-bold text-emerald-400">{offer.badge}</span>
            </div>
          </motion.div>
        ))}
      </div>
        ...
        <div className="space-y-2.5">
          {reviews.map((r, i) => (
            <motion.div
              ...
              className="bg-white/[0.06] rounded-xl p-3"
            >
              <div className="flex items-center gap-2 mb-1">
                <span className="text-[11px] font-bold text-white/70">{r.name}</span>
              ...
              <p className="text-[11px] text-white/50">{r.text}</p>
            </motion.div>
          ))}
        </div>
