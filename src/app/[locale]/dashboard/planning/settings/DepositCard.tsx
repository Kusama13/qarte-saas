...
import { Switch, ChipGroup, Callout, SettingCard } from '@/components/ui';
...
export function DepositCard({
  ...
  return (
    <SettingCard
      ...
      headerRight={<Switch checked={enabled} onChange={onEnabledChange} tone="emerald" size="md" ariaLabel={t('depositTitle')} />}
    >
      {enabled && (
        <div className="space-y-5">
          <Section number="1" title={t('depositLinkLabel')}>
            <DepositLinkInput
              ...
              error={linkMissing}
            />
            {linkMissing && <p className="text-[11px] text-red-500 mt-1.5">{t('depositLinkRequired')}</p>}
            <div className="mt-2">
              <DepositLinkInput
                value={link2}
          ...
          <Section number="2" title={t('depositAmountLabel')}>
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-[11px] font-semibold text-gray-400 uppercase">{t('or')}</span>
              <div className="flex-1 border-t border-gray-100" />
            </div>
            ...
            {amountMissing && <p className="text-[11px] text-red-500 mt-1.5">{t('depositAmountRequired')}</p>}
          </Section>
          ...
          <Section number="3" title={t('depositForWhomLabel')}>
            <div className="grid grid-cols-2 gap-1.5">
              ...
              >
                ...
                <p className={`text-[11px] font-medium mt-0.5 ${!onlyForNew ? 'text-white/70' : 'text-gray-500'}`}>
          ...
          <Section number="4" title={t('depositDeadlineLabel')}>
            ...
            {validationOpen && (
              <div className="mt-2 rounded-xl bg-emerald-50/60 border border-emerald-100 p-3 space-y-2">
                <p className="text-[11px] text-emerald-900 leading-relaxed">{t('depositValidationBody')}</p>
                <div className="pt-2 border-t border-emerald-100/80 flex items-start gap-2">
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-2">
        <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-emerald-100 text-emerald-700 text-[10px] font-bold tabular-nums">
          {number}
        </span>
