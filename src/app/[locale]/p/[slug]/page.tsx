...
const getMerchantData = cache(async (slug: string, locale: string = 'fr'): Promise<{ merchant: any; photos: any[]; services: any[]; serviceCategories: any[]; planningSlots: any[]; bookedSlots: any[]; demoOffer?: any } | null> => {
  ...
  const { data: merchant } = await supabaseAdmin
    ...
    .select(
      ...
      'welcome_offer_enabled, welcome_offer_description, welcome_offer_discount_percent, welcome_referral_code, scan_code, ' +
      ...
      'booking_mode, buffer_minutes, booking_horizon_days, booking_min_lead_hours, home_service_enabled, home_service_radius_km, shop_lat, shop_lng, hide_address_on_public_page, ' +
      'allow_customer_cancel, cancel_deadline_days, allow_customer_reschedule, reschedule_deadline_days, recurring_followup_enabled, ' +
  ...
  const [photosResult, servicesResult, categoriesResult, planningResult, contestPrizeResult] = await Promise.all([
    ...
    supabaseAdmin
      ...
      .select('id, name, price, position, category_id, duration, description, price_from, image_url')
      .eq('merchant_id', (merchant as any).id)
      .is('archived_at', null)
      .order('position'),
    supabaseAdmin
