...
export async function POST() {
  try {
    ...
    const { data: merchant } = await supabaseAdmin
      .from('merchants')
      .select('id, shop_name, reward_description, stamps_required, primary_color, logo_url, user_id, tier2_enabled, tier2_stamps_required, tier2_reward_description, loyalty_mode')
      .eq('user_id', user.id)
      .single();
    ...
    logger.error('Social kit email error:', error);
